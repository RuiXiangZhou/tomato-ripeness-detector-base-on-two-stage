...
def parser_get():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='tomato2class.pt', help='model.pt path(s)')
    ...
    for i in range (row):
        ...
        try:
            if label[i][5]==0:
                ...
                #cv2.rectangle(frame, (x, y), (x+10,y-2), color_array[0], -1, cv2.LINE_AA)  # filled
                cv2.putText(frame, 'unripe', (x, y-3), 0, 1 / 3, color_array[3], thickness=1, lineType=cv2.LINE_AA)

            if label[i][5]==1:
                frame = cv2.rectangle(frame, (x, y), (w,h), color_array[0], 2, cv2.LINE_AA)
                cv2.putText(frame, 'ripe', (x, y-2), 0, 1 / 3, color_array[0], thickness=1, lineType=cv2.LINE_AA)
